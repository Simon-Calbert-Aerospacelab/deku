...
/// single read or write it emits for a run of adjacent fields.
///
/// `DekuRead` derives it for an enum read by reading its id and nothing else:
/// all unit variants, a literal `id` on each, an unsigned primitive `id_type`,
/// a literal `bits` no wider than that type, and `Msb0`. Past 8 bits it also
/// needs an explicit big endian. Any other shape keeps its own read, so a flag
/// enum gets the trait for free but a `default` variant or an `id_pat` does not:
///
/// ```rust
/// use deku::prelude::*;
///
