...
/// here. Anything that needs the reader itself disqualifies it.
#[cfg(feature = "bits")]
fn bit_field_width(
    input: &DekuData,
    id: Option<&Id>,
    ...
    // With `id` the discriminant comes from context, not off the wire, so a run
    // read has nothing to hand over.
    if id.is_some() {
        return None;
    }
    // The rest are vetted on `DekuData`, so a new one cannot silently qualify.
    if input.any_container_set_incompatible_with_bit_field() {
        return None;
    }
    ...
    for variant in variants {
        // Vetted on `VariantData` the same way.
        if variant.any_variant_set_incompatible_with_bit_field() {
            return None;
        }
        // A literal id: one naming a context binding would not resolve inside
        // `from_bit_run`.
    ...
    // Under a byte the id cannot straddle a boundary, so byte order does not
    // come into it. Wider than that, only big-endian lines up with a run read.
    if bits > 8 {
    ...
    quote! {
        ...
        impl ::#crate_::DekuBitField for #ident #wher {
            ...
            fn to_bit_run(&self) -> core::result::Result<u64, ::#crate_::DekuError> {
                let __deku_id =
                    <Self as ::#crate_::DekuEnumExt<'_, #id_type>>::deku_id(self)? as u64;
                // The run write masks to `BITS`, so without this an `id` wider
                // than `bits` would truncate into a different variant. The
                // individual write rejects it.
                ::#crate_::writer::check_bit_size::<#ordered>(__deku_id, #bits)?;
                Ok(__deku_id)
            }
        }
    }
