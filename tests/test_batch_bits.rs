...
/// 1-bit flag, the shape bit-packed headers are full of. Endianness comes from
/// context so the flag can sit in an `endian = "big"` struct.
...
/// An `id` literal wider than the enum's `bits` must be rejected, not truncated.
///
/// The run write masks each field to its width, so without a check in
/// `to_bit_run` the surplus bits vanish and the id reads back as another
/// variant. The individual write rejects it, so a batched one must too.
#[test]
fn an_oversized_id_is_rejected_not_truncated() {
    #[derive(Copy, Clone, Debug, PartialEq, DekuRead, DekuWrite)]
    #[deku(
        id_type = "u8",
        bits = 2,
    ...
    holder!(Batched, batch_bits);
    holder!(Plain);

    // The variant that fits writes the same bytes either way.
    assert_eq!(
        Batched {
            id: Oversized::Fits,
            rest: 0b11_1101
    ...
    // The oversized one is an error either way, with the same message.
    let batched = Batched {
        id: Oversized::TooWide,
        rest: 0b11_1101,
    }
    .to_bytes();
    let plain = Plain {
    ...
    .to_bytes();
    assert!(batched.is_err(), "batched silently wrote {batched:?}");
    assert_eq!(
        format!("{:?}", batched.unwrap_err()),
        format!("{:?}", plain.unwrap_err())
    );
}
